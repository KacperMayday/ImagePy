...
class ImageMathWidget(tk.Toplevel):
    def __init__(self, source_image: ImageWindow):
        ...
        options = [getattr(MathOperators, field.name) for field in fields(MathOperators)]
    ...
    @staticmethod
    def calculate_math(list_of_pixels: list[int], math_operator: str, input_value: int) -> list[int] | None:
        match math_operator:
