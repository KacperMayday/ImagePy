...
class HistogramWidget(tk.Toplevel):
    ...
    @staticmethod
    def count_values(
        image: Image, colour_dimension: int | None = None
    ) -> dict[int, int]:
        ...
        if colour_dimension is None:
            ...
            values = [
                image.getpixel((i, j))[colour_dimension]
    ...
    def calculate_histogram_values(image: Image) -> dict[str, dict[int, int]]:
        ...
        match image.mode:
            case ImageModeEnum.GREYSCALE | ImageModeEnum.BINARY:
                histogram_values[ColourEnum.GREYSCALE] = HistogramWidget.count_values(
                    image
                )
            case ImageModeEnum.COLOUR:
                for i, colour in enumerate(
                    [ColourEnum.RED, ColourEnum.GREEN, ColourEnum.BLUE]
                ):
    ...
    def plot_histogram(self, histogram_values: dict[str, dict[int, int]]):
        ...
        if len(options) > 1:
            drop = tk.OptionMenu(
                ...
                clicked,
                *options,
                command=lambda _: show_frame(histogram_frames, clicked.get()),
            )
            drop.pack()
        ...
        for colour in histogram_values:
            ...
            histogram_canvas = HistogramCanvas(
                histogram_frame, colour, values, self.histogram_max_height
            )
            histogram_canvas.pack()
            gradient_bar = GradientBar(
                root=histogram_frame, height=10, colour_iterator=ColourIterator(colour)
            )
            gradient_bar.pack()
