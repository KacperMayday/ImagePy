...
def create_menu(root: tk.Toplevel | tk.Tk) -> tk.Menu:
    ...
    plot_profile_menu.add_command(label="Free drawing",
                                  command=lambda: ImageManager.get_focus_window().constant_move(),
                                  font=custom_font)
    plot_profile_menu.add_command(label="Line drawing",
                                  command=lambda: ImageManager.get_focus_window().point_move(),
                                  font=custom_font)
    plot_profile_menu.add_command(label="Clear",
                                  command=lambda: ImageManager.get_focus_window().clear_drawing(),
                                  font=custom_font)
    plot_profile_menu.add_separator()
    plot_profile_menu.add_command(label="Plot", command=lambda: plot_profile(ImageManager.get_focus_window()),
                                  font=custom_font)
    plot_profile_menu.add_separator()
    plot_profile_menu.add_command(label="Info", command=show_plot_info, font=custom_font)
    project_menu.add_cascade(label='Plot profile', menu=plot_profile_menu, font=custom_font)
    project_menu.add_command(label="Logic filters", command=lambda: logic_filter(ImageManager.get_focus_window()),
                             font=custom_font)
