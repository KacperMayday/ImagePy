...
class PlotProfileWidget(tk.Toplevel):
    def __init__(self, source_window: ImageWindow):
        ...
        self.checkerboard_lines_size = self.checkerboard_size // self.checkerboard_lines_interval
    ...
    def create_plot_frame(self) -> tk.Frame:
        ...
        horizontal_canvas = tk.Canvas(frame, width=self.plot_width, height=self.legend_canvas_size)
        vertical_canvas = tk.Canvas(frame, width=self.legend_canvas_size, height=self.plot_height)
        plot_canvas = tk.Canvas(frame, width=self.plot_width, height=self.plot_height, bg='white',
                                cursor="tcross")
    ...
    def draw_checkerboard(self, canvas: tk.Canvas, horizontal_canvas: tk.Canvas, vertical_canvas: tk.Canvas) -> None:
        ...
        lines_width_interval = checkerboard_width_scaled // self.checkerboard_lines_interval
        lines_height_interval = checkerboard_height_scaled // self.checkerboard_lines_interval
        offset = 5
        legend_padding = 10
        ...
        for ii, i in enumerate(range(0, self.plot_width, checkerboard_width_scaled)):
            ...
            for j in range(i + lines_width_interval,
                           i + checkerboard_width_scaled - lines_width_interval,
                           lines_width_interval):
                canvas.create_line(j, self.plot_height, j, self.plot_height - self.checkerboard_lines_length,
                                   fill='gray')
            horizontal_canvas.create_text(i + offset, legend_padding, text=ii * self.checkerboard_size)

        horizontal_canvas.create_text(self.plot_width // 2, legend_padding * 3, text='pixel distance')

        for ii, i in enumerate(range(self.plot_height - checkerboard_height_scaled, 0, -checkerboard_height_scaled)):
            canvas.create_line(0, i, self.plot_width, i, fill='gray')
            for j in range(i + checkerboard_height_scaled - lines_height_interval, i, -lines_height_interval):
                canvas.create_line(0, j, self.checkerboard_lines_length, j,
                                   fill='gray')
            vertical_canvas.create_text(self.legend_canvas_size - legend_padding, i + offset,
                                        text=(ii + 1) * self.checkerboard_size)

        # PyCharm inspection does not recognize angle argument
        # noinspection PyArgumentList
        vertical_canvas.create_text(self.legend_canvas_size - legend_padding * 3, self.plot_height // 2,
                                    text='pixel value', angle=90)
    ...
    def plot_mono_profile(self, canvas: tk.Canvas) -> None:
        ...
        for i in range(len(self.drawn_coords[:-1])):
            current_value = self.min_height + self.max_height - self.image_array[
                self.drawn_coords[i + 1]] * self.max_height / MAX_INTENSITY_LEVEL
            previous_value = self.min_height + self.max_height - self.image_array[
                self.drawn_coords[i]] * self.max_height / MAX_INTENSITY_LEVEL

            coord_x = self.plot_width * i / self.n_points
            canvas.create_line(coord_x, previous_value,
                               self.plot_width * (i + 1) / self.n_points, current_value,
    ...
    def plot_colour_profile(self, canvas: tk.Canvas) -> None:
        ...
        for j, c in enumerate(['red', 'green', 'blue']):
            for i in range(len(self.drawn_coords[:-1])):
                current_value = self.min_height + self.max_height - self.image_array[
                    self.drawn_coords[i + 1]][j] * self.max_height / MAX_INTENSITY_LEVEL
                previous_value = self.min_height + self.max_height - self.image_array[
                    self.drawn_coords[i]][j] * self.max_height / MAX_INTENSITY_LEVEL

                coord_x = self.plot_width * i / self.n_points
                canvas.create_line(coord_x, previous_value,
                                   self.plot_width * (i + 1) / self.n_points, current_value,
                                   width=self.line_width, fill=c)
    ...
    def get_pixel_value(self, pixel_distance: int) -> str | int:
        ...
        if type(pixel_value) == np.ndarray:
            pixel_value = f'red {pixel_value[0]}, green {pixel_value[1]}, blue: {pixel_value[2]}'
