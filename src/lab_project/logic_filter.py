...
class LogicFiltersWidget(tk.Toplevel):
    ...
    predicates_per_filter_type = {
        ...
        LogicFilterEnum.ISOLATED_POINTS: lambda a, b, c, d, x: a if a == b == c == d else x
    }
    ...
    @staticmethod
    def run_filter(image_array: np.array, func: Callable[[int, int, int, int, int], int]) -> np.array:
        """
        ...
        """
        target_array = np.copy(image_array)  # array copy is needed to prevent cascade pixel changing
